...
from data.models import CffrProgramRaw, CffrProgramRef

# National Priorities Project Data Repository
# load_cffr_program_ref.py 
# Created 5/10/2011
...
# destination model:  CffrProgramRef

# HOWTO:
# 1) Ensure that CffrProgramRaw is loaded and up to date
# 2) Run as Django management command from your project path "python manage.py load_cffr_program_ref"
...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        for y in CffrProgramRaw.objects.values('year').distinct():
            year_current = y['year']
            records_current = CffrProgramRef.objects.filter(year=year_current).count()
            ...
            if records_current == 0: 
                for p in CffrProgramRaw.objects.filter(year=year_current):
                    record = CffrProgramRef(year=p.year, program_code=p.program_id_code, program_name = p.program_name)
                ...
                print str(records_current) + ' CffrProgramRef records already loaded for ' + str(year_current) + '. No ' + str(year_current) + ' will be loaded.'
