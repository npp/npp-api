...
from data.models import PeoplePovertyStateRaw
        ...
        for i, row in enumerate(data_reader):
            if i == 0:
                ...
                if len(row[0]):
                    ...
                    state = row[1].lower()
                    try:
                        record = PeoplePovertyStateRaw.objects.get(year=year,state=state)
                        update_count = update_count + 1
                    except MultipleObjectsReturned:
                        print 'error: multiple records exist for %s %s' % (year, state)
                        continue
                    except:
                        record = PeoplePovertyStateRaw(year=year,state=state)
