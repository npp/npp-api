...
# Populates Energy Production model used by the API
# source model(s): EnergyProductionStateRaw, State, Msn
# source load command(s): import_energy_production
# destination model:  EnergyProductionState

# HOWTO:
# 1) Ensure that State and Msn are loaded and up to date
# 2) Run as Django management command from your project path "python manage.py load_energy_production_state"
...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        raw = EnergyProductionStateRaw.objects.filter(year__gt=1989).order_by('state')
        ...
        for r in raw:
            ...
            try:
                record = EnergyProductionState.objects.get(
                ...
                record = EnergyProductionState(year=r.year,
